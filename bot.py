...
if not all([BOT_TOKEN, GITHUB_TOKEN, ADMIN_ID, REPO_OWNER, REPO_NAME]):
    print("ERROR: Missing required environment variables!")
    print("Please set: BOT_TOKEN, GITHUB_TOKEN, ADMIN_ID, REPO_OWNER, REPO_NAME")
    exit(1)
# =======================================================
...
def start(message):
    bot.reply_to(message, "Bot စတင်ပါပြီ။ /key ဖြင့်စတင်ပါ။")
...
def handle_key(message):
    ...
    auth_list, _ = loop.run_until_complete(get_file_content("auth_list.json"))
    loop.close()
    ...
    if key in auth_list:
        ...
        if valid:
            ...
            bot.reply_to(message, "Key မှန်ကန်ပါသည်။ /input ဖြင့် Session URL ထည့်ပါ။")
        else:
            approve[message.chat.id] = False
            bot.reply_to(message, "Key Expired ဖြစ်နေပါသည်။")
    else:
        bot.reply_to(message, "သင်၏ key ကို registered မလုပ်ရသေးပါ။")
...
def listkeys(message):
    ...
    try:
        ...
        if not auth_list:
            bot.reply_to(message, "Registered key မရှိသေးပါ။")
            return
        lines = []
...
def delkey(message):
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        auth_list, sha = loop.run_until_complete(get_file_content("auth_list.json"))
        
        if user_id not in auth_list:
            loop.close()
            bot.reply_to(message, f"User ID {user_id} မတွေ့ပါ။")
            return
        del auth_list[user_id]
        loop.run_until_complete(update_file_content("auth_list.json", auth_list, sha, f"Delete key for {user_id}"))
        loop.close()
...
def genkey(message):
    ...
    try:
        ...
        loop.run_until_complete(update_file_content("auth_list.json", auth_list, sha, f"Add key for {user_id}"))
        loop.close()
...
def handle_result(message):
    ...
    if str(message.chat.id) in auth_list:
        results, _ = loop.run_until_complete(get_file_content("result.json"))
        loop.close()
        chat_id_str = str(message.chat.id)
        if chat_id_str in results and results[chat_id_str]:
            codes = "\n".join(results[chat_id_str])
            bot.reply_to(message, f"✅ Found Codes:\n{codes}")
        else:
            bot.reply_to(message, "သင့်တွင် ယခင်ကရရှိထားသော code မရှိသေးပါ။")
    else:
        loop.close()
        bot.reply_to(message, "သင်၏ key ကို registered မပြုလုပ်ရသေးပါ။")
...
def recheck(message):
    ...
    if not approve.get(chat_id, False):
        bot.reply_to(message, "/recheck ကိုအသုံးမပြုမီ /key ကိုအရင်ပြုလုပ်ပေးပါ။")
        return
    ...
    if str(message.chat.id) in auth_list:
        results, sha = loop.run_until_complete(get_file_content("result.json"))
        chat_id_str = str(message.chat.id)
        if chat_id_str in results and results[chat_id_str]:
            if message.chat.id not in user_data:
                loop.close()
                bot.reply_to(message, "/scan ကိုအသုံးမပြုမီ /key ကိုအရင်ပြုလုပ်ပေးပါ။")
                return
            if "session_url" not in user_data[message.chat.id]:
                loop.close()
                bot.reply_to(message, "/recheck ကိုအသုံးမပြုမီ /input ဖြင့် Session URL ကိုအရင်ထည့်သွင်းပေးရပါမည်။")
                return
            codes = results[chat_id_str]
            loop.close()
            bot.reply_to(message, f"Success Code များအား ပြန်လည်စစ်ဆေးနေပါသည်။")
            asyncio.create_task(recheck_codes(message, chat_id, codes, sha))
        else:
            loop.close()
            bot.reply_to(message, "သင့်တွင် success code တစ်ခုမျှမရှိသေးပါ။")
    else:
        loop.close()
        bot.reply_to(message, "သင်၏ key ကို registered မလုပ်ရသေးပါ။")

async def recheck_codes(message, chat_id, codes, sha):
    ...
    await bot.send_message(chat_id=message.chat.id, text=f"✅ Rechecked Codes:\n\n{to_show}")
    await save_rechecked_codes(str(chat_id), recheck_list, sha)
...
def handle_input(message):
    ...
    if message.chat.id in user_data:
        bot.reply_to(message, "Session URL အားစစ်ဆေးနေပါသည်။")
        asyncio.create_task(check_and_save_session(message, url))

async def check_and_save_session(message, url):
    if await check_session_url(session_url=url):
        user_data[message.chat.id]['session_url'] = url
        await bot.send_message(chat_id=message.chat.id, text="Session URL အားသိမ်းဆည်းပြီးပါပြီ။ /scan 6, 7, 8, all, ascii-lower စသည်ဖြင့်မိမိအသုံးပြုလိုတာကိုရွေးပြီး စတင်ပါ။")
    else:
        await bot.send_message(chat_id=message.chat.id, text=f"Session URL မှားယွင်းနေပါသည်။")
...
def scan(message):
    ...
    if 'session_url' not in user_data[chat_id]:
        bot.reply_to(message, "/scan ကိုအသုံးမပြုမီ /input ဖြင့် Session URL ကိုအရင်ထည့်သွင်းပေးရပါမည်။")
        return

    if chat_id in scan_tasks and not scan_tasks[chat_id]["task"].done():
        bot.reply_to(message, "/scan သည် အလုပ်လုပ်နေပြီဖြစ်သည် /scan ကိုထပ်မံမလုပ်ပါနှင့်။")
        return

    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    progress_msg = loop.run_until_complete(bot.send_message(chat_id, "🔍Scanning Codes...\n\n"))
    loop.close()
...
def stop_scan(message):
    ...
    if data and not data["task"].done():
        ...
        bot.reply_to(message, "/scan ကို ရပ်တန့်ပြီးပါပြီ။")
    else:
        bot.reply_to(message, "/stop ဖြင့်ရပ်တန့်ရန် မည်သည့်အလုပ်မျှမရှိပါ။")
