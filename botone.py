...
# ═══════════════════════════════════════════════════════════════════════════
# 🔑 CONFIGURATION - ဒီနေရာမှာ ပြောင်းထည့်ပါ
# ═══════════════════════════════════════════════════════════════════════════
...
success_messages = {}       # {chat_id: int}
limited_messages = {}       # {chat_id: int}
...
# ── Local file helpers ──────────────────────────────────────────────────────
def load_auth_list():
    """Load auth_list from local file"""
    global auth_list
    if os.path.exists(AUTH_FILE):
        try:
            with open(AUTH_FILE, 'r') as f:
                auth_list = json.load(f)
            logger.info(f"Loaded auth_list: {len(auth_list)} users")
        except Exception as e:
            logger.error(f"Error loading auth_list: {e}")
            auth_list = {}
    else:
        auth_list = {}
        save_auth_list()
...
def save_auth_list():
    """Save auth_list to local file"""
    try:
        with open(AUTH_FILE, 'w') as f:
            json.dump(auth_list, f, indent=2)
        logger.info(f"Saved auth_list: {len(auth_list)} users")
    except Exception as e:
        logger.error(f"Error saving auth_list: {e}")

def load_success_codes():
    """Load success codes from local file"""
    ...
    if os.path.exists(RESULT_FILE):
        try:
            ...
            logger.info(f"Loaded {total} success codes")
        except Exception as e:
            logger.error(f"Error loading success codes: {e}")
            success_texts = {}
    else:
        success_texts = {}

def save_success_codes():
    """Save success codes to local file"""
    try:
        data = {str(k): v for k, v in success_texts.items()}
        with open(RESULT_FILE, 'w') as f:
            json.dump(data, f, indent=2)
        logger.info(f"Saved success codes: {len(success_texts)} users")
    except Exception as e:
        logger.error(f"Error saving success codes: {e}")
...
async def status(message):
    ...
    total_codes = sum(len(v) for v in success_texts.values())
    await bot.reply_to(
        message,
        f"📊 **Bot Status**\n\n"
